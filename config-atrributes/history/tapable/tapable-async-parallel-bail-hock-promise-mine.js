...
class AsyncParallelBailHook {
  /**
   * Creates an instance of AsyncParallelBailHook.
   * @param {Array} args => ['name']
   * @memberof AsyncParallelBailHook
   */
  constructor(args) {
    this.tasks = [];
  }
...
// 注册事件
asyncParallelBailHook.tapPromise('node', (name) => {
...
asyncParallelBailHook.tapPromise('react', (name) => {
  return new Promise((resolve, reject) => {
    setTimeout(() => {
...
// 发布事件
asyncParallelBailHook.promise('bamboo').then(() => {
  console.log('end');
}).catch((err) => {
  console.log(err);
});